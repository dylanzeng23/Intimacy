...
import { todayString, nowTimeString, TYPE_COLORS, MOOD_EMOJIS } from './utils.js';
...
export async function renderEntryForm(container, editId = null, defaultDate = null) {
  ...
  const types = ['partner', 'solo', 'other'];
      ...
      <form id="entry-form" class="qlog-form">
        <div class="qlog-section">
          ...
          <div class="qlog-types" id="f-type">
            ${types.map(key => `<button type="button" class="qlog-type-btn${key === type ? ' active' : ''}" data-type="${key}"><span class="qlog-type-dot" style="background:${TYPE_COLORS[key]}"></span>${getTypeLabel(key)}</button>`).join('')}
          </div>
        </div>
        ...
        <div class="qlog-section">
          <div class="form-label">${t('entryMood')}</div>
          <div class="qlog-moods" id="f-mood">${[1,2,3,4,5].map(i => `<button type="button" class="qlog-mood-btn${i === mood ? ' active' : ''}" data-mood="${i}">${MOOD_EMOJIS[i]}</button>`).join('')}</div>
        </div>
  ...
  let selectedType = type, selectedMood = mood;
  ...
  document.getElementById('entry-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const data = { date: document.getElementById('f-date').value, time: document.getElementById('f-time').value, type: selectedType, duration: document.getElementById('f-duration').value ? parseInt(document.getElementById('f-duration').value) : null, mood: selectedMood || null, notes: document.getElementById('f-notes').value.trim() };
